...
class Logger:
    def __init__(self, file: bool = False) -> None:
        ...
        if self.file:
            if not os.path.exists("discord-logs.txt"):        
                with open("discord-logs.txt", "a", encoding='utf-8') as file:
                    file.close()
                self.log(self.LogLevel.INFO, "Created discord-logs.txt")
    ...
    def log(self, lvl: LogLevel, message: str) -> None:
        ...
        if self.file:
            with open("discord-logs.txt", "a", encoding='utf-8') as file:
                ...
                message = f"[{levels[lvl.value]}] {message}"
